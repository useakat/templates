...
C     
      INTEGER I,J,K
      REAL*8 P(0:3,NEXTERNAL),pp(0:3),ppp(0:3,nexternal)   ! four momenta. Energy is the zeroth component.
...
c     	
      IF(nincoming.EQ.1) THEN
         SQRTS=PMASS(1)
      ELSE
         SQRTS=2000d0              !CMS energy in GEV
      ENDIF
...
c
      ...
      do i=1,nexternal
         call boostx(P(0,i),pp, ppp(0,i))
      enddo

      write (*,*) "mass of referrence momentum pp:", dsqrt(dabs(DOT(pp,pp)))
      write (*,*)
      write (*,*) " Boosted phase space point:"
      write (*,*)
      write (*,*) "---------------------------------------------------"
      write (*,*)  "n        E             px             py              pz               m"
      do i=1,nexternal
      write (*,'(i2,1x,5e15.7)') i, PPP(0,i),PPP(1,i),PPP(2,i),PPP(3,i),
     .dsqrt(dabs(DOT(ppp(0,i),ppp(0,i))))
      enddo
      write (*,*) "--------------------------------------------------"

      CALL SMATRIX(PPP,MATELEM)
c
      write (*,*) "Boosted Matrix element = ", MATELEM, " GeV^",-(2*nexternal-8)	
      write (*,*) "-----------------------------------------------------------------------------"
