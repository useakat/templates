      PROGRAM DRIVER_int !KM: for 2-body decay width with integration
C**************************************************************************
...
C     
      ...
      integer nevent
      real*8 wgt,pi,Mp,beta,gam,m1,m2,mg
      parameter (pi=3.141592653589793238d0)
      Mp = 1.22089d19/dsqrt(8d0*pi)
C-----
C     BEGIN CODE
C-----
C     
...
c     	
      ...
      CALL GET_MOMENTA(SQRTS,PMASS,P,wgt) !KM
c$$$c
c$$$c	  write the information on the four momenta 
c$$$c
c$$$      write (*,*)
...
c      write (*,*) "Matrix element = ", MATELEM, " GeV^",-(2*nexternal-8)	
      ...
      gam = gam/dble(nevent)/2d0/m1/16d0/pi 
     &     *beta(m2**2/m1**2,mg**2/m1**2)
      write (*,*) "Gamma = ", gam, " GeV^",-(2*nexternal-8)

      !9503210: eq.4.64
      gam= 1d0/(384d0*pi*Mp**2) *m1**3 *(1d0-m2**2/m1**2)**4 
...
c      write (*,*) "-------------------------------------------------"
      ...
      REAL*8 function Beta(a,b)
      implicit none
      double precision a,b
        Beta = dsqrt(1d0+a**2+b**2-2d0*a-2d0*b-2d0*a*b)
      return
      end
...
C****************************************************************************
	  ...
	  SUBROUTINE GET_MOMENTA(ENERGY,PMASS,P,wgt) !KM
...
*
 ...
 1006 FORMAT(' RAMBO WARNS:',I3,' ITERATIONS DID NOT GIVE THE',
     . ' DESIRED ACCURACY =',D15.6)
      ...
      IF (INIT.EQ.1) THEN
        INIT=0
        CALL RMARIN(1802,9373) !KM (1802,9373)
      END IF
*
  10  CALL RANMAR(RAN)
