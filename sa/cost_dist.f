...
C     
      ...
      real*8 mmgro,cost,sint,cosp,sinp,beta,a,b
C     
C     EXTERNAL
C     
      REAL*8 DOT
...
c     	
      IF(nincoming.EQ.1) THEN
         SQRTS=PMASS(1)
      ELSE
         SQRTS=7000d0              !CMS energy in GEV
      ENDIF
c     CALL GET_MOMENTA(SQRTS,PMASS,P)
      n = 1000
      open(1,file="cost.dat",status="unknown")
      do j = 0,n
         cost = (2d0-1d-15)/n*j -1d0
         cosp = 1d0
         sint = dsqrt(1-cost**2)
         sinp = dsqrt(1-cosp**2)
         a = pmass(3)**2/sqrts**2
         b = pmass(4)**2/sqrts**2
         beta = dsqrt(1+a**2+b**2-2*(a+b+a*b))
         p(0,1) = sqrts/2d0*(1d0+(pmass(1)**2-pmass(2)**2)/sqrts**2)
         p(1,1) = 0d0
         p(2,1) = 0d0
         p(3,1) = p(0,1)
         p(0,2) = sqrts/2d0*(1d0+(pmass(2)**2-pmass(1)**2)/sqrts**2)
         p(1,2) = 0d0
         p(2,2) = 0d0
         p(3,2) = -p(0,2)
         p(0,3) = sqrts/2d0*(1d0+(pmass(3)**2-pmass(4)**2)/sqrts**2)
         p(1,3) = sqrts/2d0*beta*sint*cosp
         p(2,3) = sqrts/2d0*beta*sint*sinp
         p(3,3) = sqrts/2d0*beta*cost
         p(0,4) = sqrts/2d0*(1d0+(pmass(4)**2-pmass(3)**2)/sqrts**2)
         p(1,4) = -p(1,3)
         p(2,4) = -p(2,3)
         p(3,4) = -p(3,3)
...
c      write (*,*) "-----------------------------------------------------------------------------"
      ...
      write(1,'(e14.7,e22.15)') cost,MATELEM
      enddo
      close(1)
cc
