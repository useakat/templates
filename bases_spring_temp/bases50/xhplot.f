************************************************************************
*    =====================================                             *
      SUBROUTINE XHPLOT( LUN, IFG, IHIST )
*    =====================================                             *
* ((Purpose))                                                          *
...
************************************************************************
      ...
      INTEGER IP1,IP2,IP3
      INTEGER IPF,IPF2,IPX
      REAL XMIN,XMAX,DEV,FACT
      INTEGER NX,NXBIN
      REAL VMIN,VMAX,VLS,DEV2,V0,VM
      REAL VLMIN,VLMAX
      REAL VXMIN,VXMAX
      INTEGER IFLG
      REAL VLSTP,TX,VSTP,UNIT,UNITL,VX,XM,F2,ORD
      INTEGER NTOTAL,IORD,IORDR
      REAL V1,V2,VX1,ORDER,VE
      INTEGER NUMBL,NUMB,NLOW,NHIG
 ...
 9000       FORMAT(/1H1)
 9005       FORMAT(A1)
            WRITE(LUN,9100) MAPL(1,IHIST),(BUFF(I),I=IP3+1,IP3+16)
 9100       FORMAT(1X,'Histogram (ID =',I3,' ) for ',16A4)
      ELSEIF( IFG .EQ. -10 ) THEN
            ...
            ELSE
                WRITE(LUN,9005) CN
            ENDIF
            WRITE(LUN,9102) (BUFF(I),I=IP3+1,IP3+16)
 9102       FORMAT(5X,16A4)
      ELSE
            IF( IPNT .EQ. 0 ) THEN
                WRITE(LUN,9000)
            ELSE
                WRITE(LUN,9005) CN
            ENDIF
            WRITE(LUN,9105) MAPL(1,IHIST),(BUFF(I),I=IP3+1,IP3+16)
 9105       FORMAT(
  ...
  250         CONTINUE
          ...
          IF( IFG .EQ. 0 ) THEN
              ...
              IF( I .EQ. 0 .OR. I. EQ. NXBIN ) THEN
                  ...
                  F2     = VX/ORDER
                  WRITE(LUN,9300) IORD,F2,IORDR,CHARR
 9300             FORMAT(1X,'I  E',I3,' I',F6.3,8X,'E',I3,
     .                                     'I',A50)
              ELSE
                  ...
                  F2   = VX/ORDER
                  VE   = VE/ORDER
                  WRITE(LUN,9340) XM,F2,VE,IORDR,CHARR
 9340             FORMAT(1X,'I', F6.3,' I',F6.3,'+-',F5.3,' E',I3,
     .                                    'I',A50)
          ...
          ELSE
             ...
             F2     = VX/ORDER
             IF( I .EQ. 0 .OR. I .EQ. NXBIN ) THEN
                 WRITE(LUN,9400) IORD,F2,IORDR,NX,CHARR
 9400            FORMAT(1X,'I   E',I3,' I',F6.3,'E',I3,'I',
     .                                            I7,'I',A50)
             ELSE
                   XM  = (XMIN + DEV*(I - 1))/ORD
                   WRITE(LUN,9440) XM,F2,IORDR,NX,CHARR
